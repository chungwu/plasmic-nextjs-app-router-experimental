...
async function startDevServer(command: string, port: number) {
  console.log(`Plasmic: starting prepass dev server at http://localhost:${port} via "npm run ${command}"...`)

  const devServerProcess = cp.spawn(
    `npm`, ["run", command],
    {
      env: {
...
async function main() {
  const argv = await yargs(hideBin(process.argv)).option("command", {
    alias: "c",
    default: "dev",
    type: "string",
